...
func (p *ConnPool) Exec(sql string, arguments ...interface{}) (commandTag sql.Result, err error) {
    var c *Conn
    if c, err = p.Acquire(); err != nil {
        return
    }
    defer p.Release(c)
    _, err = c.Exec(sql, arguments...)
    return nil, err
}
...
func (r *Row) ScanAny(dest interface{}) error {
    ...
    scannable := utils.IsScannable(base)
...
func (p *ConnPool) Get(dest interface{}, query string, args ...interface{}) error {
    return p.QueryRow(query, args...).ScanAny(dest)
}
